...
class Employee {
	public:  // Data Members (Public Access)
		...
		float increment(float salary, int exp) {
			float inc;
			if (exp<5 ) {
				inc = salary * 0.1;  // 10% increment
			}
			else if (exp<10) {
				inc = salary * 0.2;  // 20% increment
			}
			else {
				inc = salary * 0.3;  // 30% increment
			}
			return inc + salary;
		}
		
		// Here we are passing 3 parameters (salary, experience, rating) depending on rating parameter it will calculate the increment
...
int main() {
	e1.get_info();
	cout<<"Your Final Salary: "<<e1.increment(e1.salary)<<endl;
	
	e2.get_info();
	cout<<"Your Final Salary: "<<e2.increment(e2.salary, e2.exp)<<endl;
	
	e3.get_info();
	cout<<"Your Final Salary: "<<e3.increment(e3.salary, e3.exp, e3.rating)<<endl;
